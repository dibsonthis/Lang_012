...
public:
	...
	Parser(std::vector<std::shared_ptr<Token>>& tokens, std::string file_name) : tokens(tokens), file_name(file_name)
	{
		token = tokens[0];
	}

	Parser(Lexer lexer) : tokens(lexer.tokens), file_name(lexer.get_file_name())
	{
		token = tokens[0];
	}
	...
	void error_and_skip_to(Type type, std::shared_ptr<Node> error_node, std::string message)
	{
	...
	void warn_and_skip_to(Type type, std::shared_ptr<Node> warning_node, std::string message)
	{
	...
	std::shared_ptr<Node> parse_expression(std::vector<std::shared_ptr<Node>> raw_expression)
	{
		#define ADVANCE() node = raw_expression[++index]

		std::shared_ptr<Node> node = raw_expression[0];
		int index = 0;

		std::shared_ptr<Node> root = std::make_shared<Node>(TYPE_EMPTY);
	...
	void insert_in_ast(std::shared_ptr<Node>& root, std::shared_ptr<Node>& node)
	{
		if (root == nullptr)
		{
			root = std::make_shared<Node>(TYPE_EMPTY);
		}
	...
	std::vector<std::shared_ptr<Node>> build_expression()
	{
		...
		while (token->type != TYPE_SEMICOLON)
		{
			if (token->type == TYPE_EOF)
			{
				std::shared_ptr<Node> error = std::make_shared<Node>(token);
			...
			std::shared_ptr<Node> atom = parse_atom();
			expr.push_back(atom);
			...
			{
				std::shared_ptr<Node> end_of_expression = std::make_shared<Node>(token);
	...
	std::shared_ptr<Node> parse_atom()
	{
		...
		else if (token->type == TYPE_ID && peek()->type == TYPE_LPAREN)
		{
			...
			return node;
				...
				index == 0))
		{
			std::shared_ptr<Node> node = parse_unary_op();
			return node;
		}
		else if (token->type == TYPE_LPAREN)
		{
			std::shared_ptr<Node> node = parse_paren();
			return node;
		}
		else if (token->type == TYPE_LBRACE)
		{
			std::shared_ptr<Node> node = parse_block();
			return node;
		}
		else
		{
	...
	std::shared_ptr<Node> parse_list_item()
	{
		std::shared_ptr<Node> item = std::make_shared<Node>(token);

		std::vector<std::shared_ptr<Node>> raw_expr;
		...
		while (token->type != TYPE_COMMA && token->type != TYPE_RBRACKET)
		{
			if (token->type == TYPE_EOF)
			{
				...
				return std::make_shared<Node>(TYPE_ERROR);
			}
		...
		raw_expr.push_back(std::make_shared<Node>(TYPE_END_OF_EXPRESSON));
	...
	{
		std::shared_ptr<Node> list = std::make_shared<Node>(token);
		list->type = TYPE_LIST;
		...
		while (token->type != TYPE_RBRACKET)
		{
			...
			list->list_items.push_back(item);
	...
	std::shared_ptr<Node> parse_return()
	{
		...
		std::shared_ptr<Node> expr = parse_expression(raw_expr);
		node->return_statement = expr;

		return node;
	}

	std::shared_ptr<Node> parse_func_def()
	{
		std::shared_ptr<Node> node = std::make_shared<Node>(token);
		...
		{
			std::shared_ptr<Node> param = parse_arg();
			node->func_params.push_back(param);
		...
		if (token->type == TYPE_RIGHT_ARROW)
		{
			...
			node->return_type = expr;
		}

		auto block = parse_block();
		for (std::shared_ptr<Node> expr : block->body)
		{
			node->func_body.push_back(expr);
		}

		return node;
	}

	std::shared_ptr<Node> parse_type_def()
	{
		advance();
		if (token->type != TYPE_ID)
		{
		...
		std::shared_ptr<Node> body = parse_block();
		for (std::shared_ptr<Node> expr : body->body)
		{
			node->type_body.push_back(expr);
		}

		return node;
	}

	std::shared_ptr<Node> parse_while_loop()
	{
		std::shared_ptr<Node> while_loop = std::make_shared<Node>(TYPE_WHILE);
		...
		while_loop->while_expr = parse_paren();
		...
		std::shared_ptr<Node> while_body = parse_block();
		for (auto& expr : while_body->body)
		{
			while_loop->while_body.push_back(expr);
		}

		return while_loop;
	}

	std::shared_ptr<Node> parse_if_else_atom()
	{
		std::shared_ptr<Node> if_atom = std::make_shared<Node>(token);
		...
		std::shared_ptr<Node> if_expr = parse_paren();
		if_atom->if_else_expr = if_expr;
		...
		std::shared_ptr<Node> if_body = parse_block();
		if_atom->if_else_body = if_body;

		return if_atom;
	}

	std::shared_ptr<Node> parse_if_else_statement()
	{
		std::shared_ptr<Node> if_statement = std::make_shared<Node>(token);
		if_statement->type = TYPE_IF_ELSE_STATEMENT;

		std::shared_ptr<Node> if_atom = parse_if_else_atom();

		if_statement->if_else_statements.push_back(if_atom);

		while (peek()->get_id_value() == "else" && peek(2)->get_id_value() == "if")
		{
			advance();
			advance();
			std::shared_ptr<Node> if_else_atom = parse_if_else_atom();
			if_statement->if_else_statements.push_back(if_else_atom);
		}

		if (peek()->get_id_value() == "else")
		{
			std::shared_ptr<Node> else_atom = std::make_shared<Node>(token);
			...
			else_atom->if_else_body = parse_block();
			if_statement->if_else_statements.push_back(else_atom);
		}
	...
	std::shared_ptr<Node> parse_named_block()
	{
		std::string block_name = std::make_shared<Node>(token)->get_id_value();
		advance();
		std::shared_ptr<Node> block = std::make_shared<Node>(token);
	...
	std::shared_ptr<Node> parse_paren()
	{
		advance();
		...
		while (token->type != TYPE_RPAREN)
		{
			...
			if (token->type == TYPE_LPAREN)
			{
				std::shared_ptr<Node> p_exp = parse_paren();
		...
		std::shared_ptr<Node> end_of_expression = std::make_shared<Node>(TYPE_END_OF_EXPRESSON);
		raw_expr.push_back(end_of_expression);

		std::shared_ptr<Node> node = parse_expression(raw_expr);
		node->is_p_expr = true;
		return node;
	}

	std::shared_ptr<Node> parse_unary_op()
	{
		std::shared_ptr<Node> node = std::make_shared<Node>(token);
	...
	std::shared_ptr<Node> parse_call()
	{
		...
		while (token->type != TYPE_RPAREN)
		{
			std::shared_ptr<Node> arg = parse_arg();
	...
	void parse()
	{
		...
		{
			std::vector<std::shared_ptr<Node>> raw_expr = build_expression();
			std::shared_ptr<Node> expr = parse_expression(raw_expr);
